...
define("tinymce/ui/MenuItem", [
	...
	return Widget.extend({
		...
		renderHtml: function() {
			...
			var icon = self.settings.icon, image = '';
			var imgHTML = (self._text && typeof self._text === 'string' && self._text.split('.')[1] && ['jpg', 'gif', 'png'].indexOf(self._text.split('.')[1].toLowerCase()) > -1)
                ? '<img src="' + self._value + '" alt="' + self._value + '" style="max-height:100px;max-width:100px" />'
                : '';
			...
			return (
				'<div id="' + id + '" class="' + self.classes() + '" tabindex="-1">' +
					...
					(text !== '-' ? '<span id="' + id + '-text" class="' + prefix + 'text">' +((imgHTML)?imgHTML:text) + '</span>' : '') +
