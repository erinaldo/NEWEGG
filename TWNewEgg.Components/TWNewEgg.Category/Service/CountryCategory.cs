...
{
    public class CountryCategory : BaseService
    {
        private static DateTime dateTiemNow;
        private void BrandGenerate(string PathFolder, List<int> SellerID)
        {
            ...
            BrandList = (from A in db.Product                           //Search every parentid from catagory table and parentid same with parent node, and layer is more 1 than parent node
                         where SellerID.Contains(A.SellerID) && A.Status == 0
            ...
            foreach (var item in BrandList)
            {
                ...
                var data = (from B in
                                (from A in db.Product where A.ManufactureID == item && SellerID.Contains(A.SellerID) && A.Status == 0 select A)
                ...
                SQL2XML(BrandCateTree, PathName, item); //Start save List<Category_TreeItem> into XML
                BrandCategory.Clear();


            }
        }
        public void CountryGenerate()
        {
            ...
            CountryList = (from A in db.Country select A.ID).Distinct().ToList();
            //ViewBag.BrandList = BrandList;
            dateTiemNow = DateTime.Now;
            foreach (var item in CountryList)
            {
                List<int> SellerList = (from A in db.Seller where A.CountryID == item select A.ID).ToList();
                //var productid = (from A in db.product where A.product_manufactory == item select A);
                List<Category_TreeItem> CountryCategory = new List<Category_TreeItem>();
                var data = (from B in
                                (from A in db.Product where SellerList.Contains(A.SellerID) && A.Status == 0 select A)
                            join C in db.Item.Where(x => x.Status == 0 && x.ShowOrder >= 0 && x.DateStart < dateTiemNow && x.DateEnd > dateTiemNow)
                            on B.ID equals C.ProductID
                            join D in db.Category
                            on C.CategoryID equals D.ID
                ...
                CountryCategory.AddRange(data);

                Total_Layer = Get_Total_Layer(); //Get the Max Layer from DB category table
                string PathName = "\\XML_Country\\id_" + item + "\\";
                for (int i = 0; i <= Total_Layer; i++)
                {
                    PathName = PathName + Category_type(i);
                    CreateFolder(PathName); //CreateFolder to save Xml file
                    PathName = "\\XML_Country\\id_" + item + "\\";
                }

                foreach (var node in data)
                {

                    FindParentNodes(CountryCategory, node.category_layer, 0, node.category_parentid);
                    //FindParentNodes();

                }
                List<Category_TreeItem> CountryCateTree = new List<Category_TreeItem>();
                Category_TreeItem CountryNode = new Category_TreeItem();
                CountryNode.category_id = 0;
                CountryNode.category_title = "Root";
                CountryNode.category_layer = 0;
                CountryNode.Parents = null;
                CountryNode.Nodes = new List<Category_TreeItem>();

                CountryCateTree.Add(CountryNode);
                Sav2Tree(CountryCateTree[0].Nodes, 0, -1, CountryCateTree[0], CountryCategory); //Start set the data from DB into List<Category_TreeItem>, give parentid and layerid to build the tree
                ...
                SQL2XML(CountryCateTree, PathName); //Start save List<Category_TreeItem> into XML
                CountryCategory.Clear();


                BrandGenerate(PathName, SellerList);
