...
{
    public class ConvenienceStorePayGetService : IPayTypeGetService
    {
        private IPayTypeRepoAdapter _iPayTypeRepoAdapter;

        public ConvenienceStorePayGetService(IPayTypeRepoAdapter iPayTypeRepoAdapter) 
        {
        ...
        public IEnumerable<Models.DomainModels.CartPayment.DM_PayType> GetPayTypes(Models.DomainModels.ShoppingCartPayType.DM_ShoppingCart shoppingCart)
        {
            ...
            querySearch = this._iPayTypeRepoAdapter.getAll().Where(x => x.PaymentTermID == PaymentTermID.超商付款 && x.Status == 0).OrderBy(x => x.ChooseOrder).ToList();
