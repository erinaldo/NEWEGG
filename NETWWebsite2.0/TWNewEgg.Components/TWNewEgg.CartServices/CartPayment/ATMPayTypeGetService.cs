...
{
    public class ATMPayTypeGetService : IPayTypeGetService
    {
        private IPayTypeRepoAdapter _iPayTypeRepoAdapter;

        public ATMPayTypeGetService(IPayTypeRepoAdapter iPayTypeRepoAdapter) 
        {
        ...
        IEnumerable<Models.DomainModels.CartPayment.DM_PayType> IPayTypeGetService.GetPayTypes(DM_ShoppingCart shoppingCart)
        {
            ...
            querySearch = this._iPayTypeRepoAdapter.getAll().Where(x => x.PaymentTermID == PaymentTermID.實體ATM && x.Status == 0);
