...
{
    /// <summary>
    ///2 節日問候卡 -----------------------add by bruce 20160329
    /// </summary>
    public class HolidayGreetingWordsService : IHolidayGreetingWordsService
    {
        /// <summary>
        /// db a table adapter
        /// </summary>
        ...
        /// 2 節日問候卡
        /// </summary>
        private int _category_id = 2;

        public HolidayGreetingWordsService(IGreetingWordsRepoAdapter adapter)
        {
            _adapter = adapter;
        }
        ...
        /// <returns></returns>
        public HolidayGreetingWordsDM GetSingle(int? id)
        {
            try
            {
                HolidayGreetingWordsDM domain_info = null;
                var db_info = this._adapter.GetAll().FirstOrDefault(x => x.ID == id);
                if (db_info != null)
                {
                    domain_info = ModelConverter.ConvertTo<HolidayGreetingWordsDM>(db_info);
                    return domain_info;
                }
                return null;
            }
        ...
        /// <param name="id">系統流水編號</param>
        /// <returns></returns>
        public HolidayGreetingWordsDM GetInfo(int? id)
        {
        ...
        /// <returns></returns>
        public List<HolidayGreetingWordsDM> GetData(string description, int? showall)
        {
            ...
            int category_id = _category_id;
            List<HolidayGreetingWordsDM> list_dm = null;
            List<GreetingWordsDB> list_db_result = null;
            try
            {
                string codetext = string.Empty;
                ...
                list_db_result = list_db.ToList();
                if (list_db_result != null)
                    list_dm = ModelConverter.ConvertTo<List<HolidayGreetingWordsDM>>(list_db_result);
            }
            catch (Exception ex)
            {
                throw new NotImplementedException(ex.Message, ex);
        ...
        /// </summary>
        /// <returns></returns>
        public List<HolidayGreetingWordsDM> GetAll()
        {
            int category_id = _category_id;
            List<HolidayGreetingWordsDM> list_dm = null;
            List<GreetingWordsDB> list_db_result = null;
        ...
        /// </param>
        /// <returns></returns>
        public List<HolidayGreetingWordsDM> GetShow(DateTime now_date)
        {
            int category_id = _category_id;
        ...
        /// <returns></returns>
        public bool Update(HolidayGreetingWordsDM info, string user_name)
        {
        ...
        private string change_path_to_pic(HolidayGreetingWordsDM info)
        {
            //圖片儲存至Pic底下
        ...
        /// <param name="info">domain model</param>
        /// <returns></returns>
        public HolidayGreetingWordsDM Add(HolidayGreetingWordsDM info, string user_name)
        {
            ...
            try
            {
                ...
                int show_order = 0;
                foreach (HolidayGreetingWordsDM each_info in data)
                ...
                info = ModelConverter.ConvertTo<HolidayGreetingWordsDM>(db_info);
        ...
        ///// <returns></returns>
        //public List<HolidayGreetingWordsDM> Add(List<HolidayGreetingWordsDM> list_info)
        //{
        //    List<HolidayGreetingWordsDM> list_result = new List<HolidayGreetingWordsDM>();
        //    if (list_info == null) return list_result;
        //    int category_id = _category_id;
        //    foreach (HolidayGreetingWordsDM each_info in list_info)
        //    {
        ...
        public List<bool> Save(List<HolidayGreetingWordsDM> list_info, string user_name)
        {
            ...
            foreach (HolidayGreetingWordsDM each_info in list_info)
            {
                ...
                {
                    each_info.CreateDate = DateTime.UtcNow.AddHours(8);
                    HolidayGreetingWordsDM info = null;
        ...
        ///// <param name="info">domain model</param>
        ///// <returns></returns>
        //public bool Del(HolidayGreetingWordsDM info)
        //{
        //    bool is_ok = false;
        //    if (info == null) return is_ok;
        //    try
