...
{
    public class BrandCategory : BaseService
    {
        private static DateTime dateTiemNow;
        public void BrandGenerate()
        {
            List<int> BrandList = new List<int>();
            BrandList = (from A in db.Product
                         where A.Status == 0
                         select A.ManufactureID).Distinct().ToList();
            //BrandList = (from A in db.Manufacture select A.manufacture_id).Distinct().ToList();
            ...
            foreach (var item in BrandList)
            {
                ...
                var data = (from B in
                                (from A in db.Product where A.ManufactureID == item && A.Status == 0 select A)
